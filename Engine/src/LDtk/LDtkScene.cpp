...
namespace Umbra {
    ...
    namespace {
        ...
        String ResolveTilesetPath(const String& _ldtkPath, const std::string& _relPath) {
            return PathUtils::Join(PathUtils::GetDirectory(_ldtkPath), _relPath);
        }
        ...
        void ParseWorld(
            const ldtk::World& _world, const String& _ldtkPath, const String& _levelName, LDtkLoadData& _data) {
            for (const auto& level : _world.allLevels()) {
                ...
                for (const auto& layer : layers) {
                    ...
                    if (type == ldtk::LayerType::Tiles || type == ldtk::LayerType::AutoLayer ||
                        ...
                        if (!found) {
                            _data.uniqueTilesetPaths.push_back(fullPath);
                        }
                        ...
                        for (const auto& tile : layer.allTiles()) {
                            ...
                            float uvX = static_cast<float>(tr.x) / tsW;
                            float uvY = static_cast<float>(tr.y) / tsH;
                            float uvW = static_cast<float>(tr.width) / tsW;
                            float uvH = static_cast<float>(tr.height) / tsH;
                            // Horizontal flip: swap left/right by shifting origin to the far edge
                            if (tile.flipX) { uvX += uvW; uvW = -uvW; }
                            // Vertical flip: swap top/bottom
                            if (tile.flipY) { uvY += uvH; uvH = -uvH; }
                            entry.uvX         = uvX;
                            entry.uvY         = uvY;
                            entry.uvW         = uvW;
                            entry.uvH         = uvH;
    ...
    LDtkScene::LDtkScene(const String& _ldtkPath, const String& _levelName)
        : mLdtkPath(_ldtkPath), mLevelName(_levelName) {}
    ...
    void LDtkScene::OnUpdate() {
        ...
        if (!mLoadData || !mLoadData->bReady.Load(EMemoryOrder::Acquire)) {
            return;
        }
        SpawnECSEntities();
    ...
    void LDtkScene::SpawnECSEntities() {
        ...
        for (const auto& tile : data.tiles) {
            ...
            world->AddComponent<TransformComponent>(
                e, TransformComponent(Math::Vector2f(tile.x, tile.y), Math::Vector2f(tile.width, tile.height)));
        ...
        for (const auto& ent : data.entities) {
            EntityID e = world->CreateEntity();
            world->AddComponent<TransformComponent>(
                e, TransformComponent(Math::Vector2f(ent.x, ent.y), Math::Vector2f(ent.width, ent.height)));
